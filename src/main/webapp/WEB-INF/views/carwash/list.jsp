...
<fieldset class="table_container">
    <legend>
        <spring:message code="title.carwash.list" />
			...
			<c:forEach var="order" items="${orders}">
				<c:choose>
					<c:when test="${order == 'carwashtype.name'}">
						<td>${data.carWashType.name}</td>
					</c:when>
					<c:when test="${order == 'carwash.datebeginwash'}">
						<td><fmt:formatDate pattern="dd-MM-yyyy" value="${data.dateBeginWash}" /></td>
					</c:when>
					<c:when test="${order == 'carwash.dateendwash'}">
						<td><fmt:formatDate pattern="dd-MM-yyyy" value="${data.dateEndWash}" /></td>
					</c:when>
				</c:choose>
			</c:forEach>
			<td>
				<div class="table-options">
					<a href="<c:url value="/admin/carwash/edit/${data.id}" />" class="btn btn-mini" data-toggle="tooltip" data-original-title="<spring:message code="label.edit" />"><span class="icon-edit"></span></a>
					<a href="<c:url value="/admin/carwash/delete/${data.id}" />" class="btn btn-danger btn-mini btn-confirm" data-toggle="tooltip" data-original-title="<spring:message code="label.delete" />"><span class="icon-white icon-trash"></span></a>
				</div>
			</td>
		</tr>
