...
<form:form method="POST" modelAttribute="refuelingTypeAddForm" class="form-horizontal" id="form-add">
	...
	<div class="well form-title">
		...
		<h3><spring:message code="title.petrol.refuelingtype.add" /></h3>
	</div>
	<div id="form-add-data">
		<div class="control-group<spring:bind path="name"><c:if test="${status.error}"> error</c:if></spring:bind>">
			<label class="control-label required" for="name"><spring:message code="label.petrol.refuelingtype.name" />:</label>
			<div class="controls">
		...
		<div class="control-group<spring:bind path="cost"><c:if test="${status.error}"> error</c:if></spring:bind>">
			<label class="control-label required" for="cost"><spring:message code="label.petrol.refuelingtype.cost" />:</label>
			<div class="controls">
		...
		<div class="control-group<spring:bind path="loyalityPoints"><c:if test="${status.error}"> error</c:if></spring:bind>">
			<label class="control-label required" for="loyalityPoints"><spring:message code="label.petrol.refuelingtype.loyalitypoints" />:</label>
			<div class="controls">
