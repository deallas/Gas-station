...
<form:form method="POST" modelAttribute="accountEditForm" class="form-horizontal" id="form-add">
	...
	<div class="well form-title">
		...
		<h3><spring:message code="label.client.edit" /></h3>
	</div>
	<div id="form-add-data">
		...
		<div class="control-group<spring:bind path="confirmPassword"><c:if test="${status.error}"> error</c:if></spring:bind>">
			<label class="control-label" for="confirmPassword"><spring:message code="label.confirmpassword" />:</label>
			<div class="controls">
				<form:password path="confirmPassword" placeholder="..." />
				<span class="help-inline"><form:errors path="confirmPassword" /></span>
			</div>
		</div>
		<div class="control-group<spring:bind path="points"><c:if test="${status.error}"> error</c:if></spring:bind>">
			<label class="control-label required" for="points"><spring:message code="label.points" />:</label>
			<div class="controls">
				<form:input path="points" placeholder="..." />
				<span class="help-inline"><form:errors path="points" /></span>
			</div>
		</div>
		<div class="control-group<spring:bind path="status"><c:if test="${status.error}"> error</c:if></spring:bind>">
			...
			<div class="controls">
				<form:select path="status">
					<form:option value="BANNED" ><spring:message code="label.status.banned" /></form:option>
